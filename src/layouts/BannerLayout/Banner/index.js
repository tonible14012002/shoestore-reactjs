...
const Banner = () => {
    ...
    const bgColorsRef = useRef(['bg-gradient-to-r from-pink-600 to-red-500', 'bg-gradient-to-r from-violet-500 to-fuchsia-500',
...
const BannerItem = ({backGroundColor, header, subHeader, link, title, ...props}) => {
    return (
        <div className={`${backGroundColor} rounded-3xl ml-2 mr-2 flex justify-between `}
            {...props}
        >
            <div className=" w-full h-full tablet:pt-5 tablet:pl-14 pt-4 pl-5 pr-5 pb-4" >
                ...
                <p className="w-full max-h-[100px] text-white max-w-[600px]" > {subHeader}</p>
                <Button className="rounded-3xl mt-4 pt-2.5 pb-2.5 pl-5 pr-5 text-center text-sm font-semibold bg-neutral-800 shadow-xl text-white">{title}</Button>
            </div>
